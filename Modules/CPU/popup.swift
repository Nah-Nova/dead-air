...
internal class Popup: PopupWrapper {
    ...
    private var systemColorState: SColor = .oxide
    private var systemColor: NSColor { self.systemColorState.additional as? NSColor ?? Brand.stateDead }
    private var userColorState: SColor = .signalTeal
    private var userColor: NSColor { self.userColorState.additional as? NSColor ?? Brand.stateLive }
    private var idleColorState: SColor = .lightGray
    private var idleColor: NSColor { self.idleColorState.additional as? NSColor ?? Brand.legendSecondary }
    private var chartColorState: SColor = .signalTeal
    private var chartColor: NSColor { self.chartColorState.additional as? NSColor ?? Brand.stateLive }
    private var eCoresColorState: SColor = .signalTeal
    private var eCoresColor: NSColor { self.eCoresColorState.additional as? NSColor ?? Brand.stateLive }
    private var pCoresColorState: SColor = .oxide
    private var pCoresColor: NSColor { self.pCoresColorState.additional as? NSColor ?? Brand.stateDead }
    private var sCoresColorState: SColor = .tallyAmber
    private var sCoresColor: NSColor { self.sCoresColorState.additional as? NSColor ?? Brand.lampLit }
    ...
    private func renderLoad(_ value: CPU_Load) {
        ...
        if let cores = SystemKit.shared.device.info.cpu?.cores, !cores.isEmpty {
            ...
            for i in 0..<value.usagePerCore.count {
                usagePerCore.append(ColorValue(value.usagePerCore[i], color: Brand.stateLive))
            }
        }
