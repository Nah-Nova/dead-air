...
final class MicController {
    ...
    private func applyVolumeFallback(_ device: AudioDeviceID, muted: Bool) {
        ...
        if muted {
            ...
            for element in elements {
                // No saved value means the input was already at zero before we touched it,
                // so restore zero. Guessing 0.75 raised the gain on a user who had
                // deliberately silenced the device in System Settings.
                _ = Self.setVolume(device, element: element, value: savedVolumes[element] ?? 0)
            }
            savedVolumes.removeAll()
        }
    }
