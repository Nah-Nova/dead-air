...
internal class Preview: PreviewWrapper {
    ...
    private var appColorState: SColor = .signalTeal
    private var appColor: NSColor { self.appColorState.additional as? NSColor ?? Brand.stateLive }
    private var wiredColorState: SColor = .tallyAmber
    private var wiredColor: NSColor { self.wiredColorState.additional as? NSColor ?? Brand.lampLit }
    private var compressedColorState: SColor = .oxide
    private var compressedColor: NSColor { self.compressedColorState.additional as? NSColor ?? Brand.stateDead }
    private var freeColorState: SColor = .lightGray
    private var freeColor: NSColor { self.freeColorState.additional as? NSColor ?? Brand.legendSecondary }
    ...
    private func pressureView() -> NSView {
        ...
        let circle = GaugeChartView(segments: [
            ColorValue(1/3, color: Brand.stateLive),
            ColorValue(1/3, color: Brand.lampLit),
            ColorValue(1/3, color: Brand.stateDead)
