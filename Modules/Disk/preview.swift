...
internal class Preview: PreviewWrapper {
    ...
    private var readColorState: SColor = .signalTeal
    private var readColor: NSColor { self.readColorState.additional as? NSColor ?? Brand.stateLive }
    private var writeColorState: SColor = .oxide
    private var writeColor: NSColor { self.writeColorState.additional as? NSColor ?? Brand.stateDead }
    ...
    private func usageView() -> NSView {
        ...
        let details: NSView = {
            ...
            let title: NSView = {
                ...
                readState.layer?.backgroundColor = Brand.legendSecondary.withAlphaComponent(0.75).cgColor
                ...
                writeState.layer?.backgroundColor = Brand.legendSecondary.withAlphaComponent(0.75).cgColor
            ...
            self.usedField = previewRow(levels, space: false, color: Brand.stateLive, title: "\(localizedString("Used")):", value: "")
            self.freeField = previewRow(levels, space: false, color: Brand.legendSecondary, title: "\(localizedString("Free")):", value: "")
                ...
                if let main = self.main, let update = value.first(where: { $0.uuid == main.id }) {
                    ...
                    if let smart = update.smart {
                        ...
                        if let warning = smart.criticalWarning {
                            ...
                            self.criticalWarningValueField?.textColor = list.isEmpty ? .textColor : Brand.stateDead
                        } else {
                        ...
                        if let spare = smart.availableSpare {
                            ...
                            if let threshold = smart.spareThreshold {
                                self.availableSpareValueField?.textColor = spare < threshold ? Brand.stateDead : .textColor
                        ...
                        if let mediaErrors = smart.mediaErrors {
                            ...
                            self.mediaErrorsValueField?.textColor = mediaErrors > 0 ? Brand.stateDead : .textColor
                        } else {
    ...
    internal func activityCallback(_ value: Disks) {
        ...
        self.readState?.layer?.backgroundColor = read != 0 ? self.readColor.cgColor : Brand.legendSecondary.withAlphaComponent(0.75).cgColor
        
        self.writeState?.toolTip = "Write: \(Units(bytes: write).getReadableSpeed(base: self.base, unit: self.speedUnit))"
        self.writeState?.layer?.backgroundColor = write != 0 ? self.writeColor.cgColor : Brand.legendSecondary.withAlphaComponent(0.75).cgColor
