...
public class Portal: PortalWrapper {
    ...
    private var downloadColor: NSColor {
        let v = SColor.fromString(Store.shared.string(key: "\(self.name)_downloadColor", defaultValue: SColor.signalTeal.key))
        var value = Brand.stateLive
    ...
    private var uploadColor: NSColor {
        let v = SColor.fromString(Store.shared.string(key: "\(self.name)_uploadColor", defaultValue: SColor.oxide.key))
        var value = Brand.stateDead
        if let color = v.additional as? NSColor {
            value = color
        }
        return value
