...
private class CalendarItemView: NSView {
    ...
    init(_ components: DateComponents, size: NSSize, isToday: Bool, isOutOfMonth: Bool) {
        ...
        if isToday {
            ...
            circle.layer?.backgroundColor = Brand.stateDead.cgColor
            circle.layer?.cornerRadius = circleSize/2
            
            field.textColor = Brand.inkOnState
            ...
            if isOutOfMonth {
                field.textColor = Brand.legendSecondary
            }
            self.addSubview(field)
            NSLayoutConstraint.activate([
                field.centerXAnchor.constraint(equalTo: self.centerXAnchor),
...
internal class ClockChart: NSView {
    ...
    public override func draw(_ dirtyRect: NSRect) {
        ...
        self.secondsLayer.backgroundColor = Brand.stateDead.cgColor
