...
class Helper: NSObject, NSXPCListenerDelegate, HelperProtocol {
    ...
    private let smcQueue = DispatchQueue(label: "nu.soep.deadair.SMC.Helper.smcQueue")
    ...
    override init() {
        self.listener = NSXPCListener(machServiceName: "nu.soep.deadair.SMC.Helper")
        super.init()
    ...
    private func uninstallHelper() {
        ...
        process.arguments = ["unload", "/Library/LaunchDaemons/nu.soep.deadair.SMC.Helper.plist"]
        do {
            try process.run()
        ...
        do {
            try FileManager.default.removeItem(at: URL(fileURLWithPath: "/Library/LaunchDaemons/nu.soep.deadair.SMC.Helper.plist"))
        } catch let err {
        ...
        do {
            try FileManager.default.removeItem(at: URL(fileURLWithPath: "/Library/PrivilegedHelperTools/nu.soep.deadair.SMC.Helper"))
        } catch let err {
...
extension Helper {
    ...
    func uninstall() {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: "/Library/PrivilegedHelperTools/nu.soep.deadair.SMC.Helper")
        process.qualityOfService = QualityOfService.userInitiated
...
struct CodesignCheck {
    ...
    public static func codeSigningMatches(auditToken token: audit_token_t) throws -> Bool {
        ...
        // An empty chain must never satisfy the comparison. Both an ad-hoc signed helper
        // and an ad-hoc signed caller produce [], and [] == [] would admit any local
        // process to a root daemon.
        guard !selfCerts.isEmpty else { return false }
        return selfCerts == (try self.codeSigningCertificates(forAuditToken: token))
    }
