...
public class SystemStats {
    ...
    public func login() {
        self.auth.login { url in
            ...
            debug("Open \(url) to login to Dead Air Remote", log: self.log)
    ...
    public func logout() {
        ...
        debug("Logout successfully from Dead Air Remote", log: self.log)
        NotificationCenter.default.post(name: .remoteState, object: nil, userInfo: ["auth": self.isAuthorized])
    }
    
    public func deregister() {
        // Dead Air: the remote reporting service is removed, no request is sent.
        self.logout()
    }

    public func send(key: String, value: Any) {
        // Dead Air: the remote reporting service is removed, nothing is published.
        return
    }
    ...
    @objc private func successLogin() {
        ...
        NotificationCenter.default.post(name: .remoteState, object: nil, userInfo: ["auth": self.isAuthorized])
        self.mqtt.connect()
        debug("Login successfully on Dead Air Remote", log: self.log)
    }
    ...
    private func registerDevice(omitCooldown: Bool = false) {
        // Dead Air: the remote reporting service is removed, no device data leaves the machine.
    }

    private func fetchAccount() {
        // Dead Air: the remote reporting service is removed, no account is fetched.
    }
...
public class RemoteAuth {
    ...
    private func validate(_ completion: @escaping (Bool) -> Void) {
        // Dead Air: the remote auth service is removed, no request is sent.
        completion(false)
    }
...
class MQTTManager: NSObject {
    ...
    private let stateQueue = DispatchQueue(label: "nu.soep.deadair.Remote.MQTT")
    ...
    public func connect() {
        // Dead Air: the remote broker is removed, no connection is opened.
        return
    }
...
enum RemoteKeychain {
    private static let service: String = (Bundle.main.bundleIdentifier ?? "nu.soep.deadair") + ".remote"
