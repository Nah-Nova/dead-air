...
public class DB {
    ...
    private let queue = DispatchQueue(label: "nu.soep.deadair.db")
    ...
    init() {
        ...
        let supportPath = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("DeadAir")
        let tmpPath = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("DeadAir")
