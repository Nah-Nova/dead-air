...
extension SColor: CaseIterable {
    ...
    public static var signalTeal: SColor { SColor(key: "signalTeal", value: "Signal teal", additional: Brand.stateLive) }
    public static var oxide: SColor { SColor(key: "oxide", value: "Oxide", additional: Brand.stateDead) }
    /// Deliberately absent from `allColors`: that list populates the widget colour pickers,
    /// which set type, and amber may never set type. Code may still use it as a fill default.
    public static var tallyAmber: SColor { SColor(key: "tallyAmber", value: "Tally amber", additional: Brand.lampLit) }
    ...
    public static var allCases: [SColor] {
        return [.utilization, .pressure, .cluster, separator1,
                .systemAccent, .monochrome, .signalTeal, .oxide, .tallyAmber, separator2,
    ...
    public static var allColors: [SColor] {
        return [.systemAccent, .monochrome, .signalTeal, .oxide, .separator2, .clear, .white, .black, .gray, .secondGray, .darkGray, .lightGray,
                .red, .secondRed, .green, .secondGreen, .blue, .secondBlue, .yellow, .secondYellow,
                .orange, .secondOrange, .purple, .secondPurple, .brown, .secondBrown,
                .cyan, .magenta, .pink, .teal, .indigo
        ]
...
public enum RAMPressure: String, Codable {
    ...
    public func pressureColor() -> NSColor {
        switch self {
        case .normal:
            return Brand.stateLive
        case .warning:
            return Brand.lampLit
        case .critical:
            return Brand.stateDead
        }
    }
    
    /// Pressure as type. Amber never sets type, so warning reads neutral and only
    /// critical takes the accent.
    public func pressureInk() -> NSColor {
        switch self {
        case .normal, .warning:
            return Brand.legendPrimary
        case .critical:
            return Brand.stateDead
        }
    }
