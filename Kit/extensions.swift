...
public extension Double {
    ...
    func usageColor(zones: colorZones = (0.6, 0.8), reversed: Bool = false) -> NSColor {
        let firstColor: NSColor = Brand.stateLive
        let secondColor: NSColor = Brand.lampLit
        let thirdColor: NSColor = Brand.stateDead
    ...
    /// The same zones as `usageColor`, but for type. Tally amber is a lamp and must
    /// never set type, so a warning zone reads as neutral ink here and only the
    /// critical zone takes the accent. Use this wherever the result colours a string;
    /// use `usageColor` for bars, gauges and other fills, where amber is legal.
    func usageInk(zones: colorZones = (0.6, 0.8), reversed: Bool = false) -> NSColor {
        let critical: Bool
        if reversed {
            critical = self <= zones.orange
        } else {
            critical = self > zones.red
        }
        return critical ? Brand.stateDead : Brand.legendPrimary
    }
...
public extension NSView {
    ...
    func sliderView(action: Selector, value: Int, initialValue: String, min: Double = 1, max: Double = 100, valueWidth: CGFloat = 40) -> NSView {
        ...
        valueField.font = Brand.Font.utility(size: 12)
...
public class KeyboardShartcutView: NSStackView {
    ...
    public init(callback: @escaping (_ value: [UInt16]) -> Void, value: [UInt16]) {
        ...
        valueField.font = Brand.Font.utility(size: 13)
        valueField.textColor = .textColor
        valueField.alignment = .center
