...
public class MemoryWidget: WidgetWrapper {
    ...
    public override func draw(_ dirtyRect: NSRect) {
        ...
        var attributes = [
            NSAttributedString.Key.font: Brand.Font.utility(size: 9, weight: .light),
        ...
        var freeColor: NSColor = Brand.stateLive
        var usedColor: NSColor = Brand.stateLive
        switch self.colorState {
        case .systemAccent: 
            freeColor = .controlAccentColor
            usedColor = .controlAccentColor
        case .utilization:
            freeColor = (1 - percentage).usageInk()
            usedColor = percentage.usageInk()
        case .pressure:
            usedColor = pressureLevel.pressureInk()
            freeColor = pressureLevel.pressureInk()
        case .monochrome:
            freeColor = (isDarkMode ? NSColor.white : NSColor.black)
            usedColor = (isDarkMode ? NSColor.white : NSColor.black)
        default:
            freeColor = self.colorState.additional as? NSColor ?? Brand.stateLive
            usedColor = self.colorState.additional as? NSColor ?? Brand.stateLive
        }
