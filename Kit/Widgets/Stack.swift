...
public class StackWidget: WidgetWrapper {
    ...
    private var monospacedFontState: Bool = true
    ...
    private func drawOneRow(_ x: CGFloat, _ element: Stack_t) -> CGFloat {
        ...
        var font: NSFont = Brand.Font.body(size: 13, weight: .regular)
        if monospacedFontState {
            font = Brand.Font.utility(size: 13, weight: .regular)
        }
    ...
    private func drawTwoRows(_ x: CGFloat, _ topElement: Stack_t, _ bottomElement: Stack_t?) -> CGFloat {
        ...
        var font: NSFont
        if monospacedFontState {
            font = Brand.Font.utility(size: 10, weight: .light)
        } else {
            font = Brand.Font.body(size: 10, weight: .light)
        }
        let style = NSMutableParagraphStyle()
        style.alignment = alignment
