...
public class SpeedWidget: WidgetWrapper {
    ...
    private var inputColor: (String) -> NSColor {{ state in
        if state == "none" { return Brand.legendPrimary }
    ...
    private var outputColor: (String) -> NSColor {{ state in
        if state == "none" { return Brand.legendPrimary }
        var color = self.monochromeState ? MonochromeColor.red : (self.outputColorState.additional as? NSColor ?? Brand.stateDead)
