...
public struct module_c {
    ...
    init(in path: String) {
        ...
        // A Dead Air mark wins over a system symbol: ten marks in one hand read as a
        // set, ten borrowed symbols read as someone else's app. Symbol stays as the
        // fallback for a module that has not been drawn yet.
        if let mark = dict["Mark"] as? String {
            self.icon = Brand.moduleMark(mark)
        }
        if self.icon == nil, let symbol = dict["Symbol"] as? String {
...
open class Module {
    /// The shared suite the widget extension reads. Ad-hoc builds have no team prefix, so
    /// a missing TeamId degrades to the app's own defaults instead of crashing on launch.
    public static var widgetSuiteName: String {
        let prefix = Bundle.main.object(forInfoDictionaryKey: "TeamId") as? String ?? ""
        return prefix.isEmpty ? "nu.soep.deadair.widgets" : "\(prefix).nu.soep.deadair.widgets"
    }
    ...
    public var userDefaults: UserDefaults? = UserDefaults(suiteName: Module.widgetSuiteName)
