...
public class LabelField: NSTextField {
    public init(frame: NSRect = NSRect.zero, _ label: String = "", size: CGFloat = 12) {
        ...
        self.textColor = Brand.legendSecondary
        self.alignment = .natural
        self.font = Brand.Font.body(size: size)
...
public class ValueField: NSTextField {
    public init(frame: NSRect = NSRect.zero, _ value: String = "") {
        ...
        // A value in a popup row is always something a machine printed, a level, a
        // temperature, a speed, so it takes the utility face on a fixed advance and
        // stops shuffling as it ticks.
        self.stringValue = value
        self.textColor = Brand.legendPrimary
        self.alignment = .right
        self.font = Brand.Font.utility(size: 12, weight: .regular)
...
public func separatorView(_ title: String, origin: NSPoint = NSPoint(x: 0, y: 0), width: CGFloat = 0, rightInset: CGFloat = 0) -> NSView {
    // A quiet leading section label, no scored lines: the section break is carried by
    // space and the label alone, which is what keeps the popups reading calm.
    ...
    labelView.attributedStringValue = Brand.cardTitle(title)
    ...
    NSLayoutConstraint.activate([
        labelView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 2),
        labelView.trailingAnchor.constraint(lessThanOrEqualTo: view.trailingAnchor, constant: -rightInset),
        labelView.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -6)
    ])
...
public func portalRow(_ v: NSStackView, title: String, value: String = "", isSelectable: Bool = false) -> (LabelField, ValueField, NSStackView) {
    ...
    valueView.font = Brand.Font.utility(size: 12)
...
/// True when `latestVersion` is a release the user on `currentVersion` should be offered.
///
/// Rewritten because the original indexed the split components directly, so any version
/// with fewer than three parts crashed the update check, and its cascade of comparisons
/// answered true for two identical pre-releases. The expectations live in `Tests/Kit.swift`.
///
/// Ordering is the ordinary one on major, minor and patch, and on a tie a release outranks
/// any pre-release of the same number, so `2.11.0-beta1` is older than `2.11.0`. A user on
/// a release is never offered a pre-release, even a numerically newer one.
public func isNewestVersion(currentVersion: String, latestVersion: String) -> Bool {
    guard let current = parseVersion(currentVersion), let latest = parseVersion(latestVersion) else {
        return false
    }
    if current.beta == nil && latest.beta != nil {
        return false
    }
    return versionRank(latest) > versionRank(current)
}

/// `nil` for anything with no leading number at all, which is how an empty string and a
/// failed request are both refused rather than read as 0.0.0.
internal func parseVersion(_ value: String) -> Version? {
    var text = value.condenseWhitespace()
    ...
    func number(_ index: Int) -> Int {
        ...
        // The last component can carry a pre-release suffix, as in "0-beta2".
        let head = parts[index].split(separator: "-").first ?? ""
        return Int(head) ?? 0
    }

    var version = Version(major: number(0), minor: number(1), patch: number(2))
    if let last = parts.last, last.contains("-") {
        let suffix = last.split(separator: "-").dropFirst().joined(separator: "-")
        version.beta = Int(suffix.replacingOccurrences(of: "beta", with: "")) ?? 0
    }
    return version
}

/// A single comparable value. A release sorts above every pre-release of the same number,
/// which is what `Int.max` in the last position buys.
private func versionRank(_ version: Version) -> [Int] {
    [version.major, version.minor, version.patch, version.beta ?? Int.max]
}
...
public class SMCHelper {
    ...
    private let id: String = "nu.soep.deadair.SMC.Helper"
    private let plistName: String = "nu.soep.deadair.SMC.Helper.plist"
    ...
    public func checkForUpdate() {
        ...
        let helperURL = Bundle.main.bundleURL.appendingPathComponent("Contents/Library/LaunchServices/nu.soep.deadair.SMC.Helper")
    ...
    private func installLegacy(completion: @escaping (_ state: SMCHelperInstallState) -> Void) {
        ...
        if SMJobBless(kSMDomainSystemLaunchd, "nu.soep.deadair.SMC.Helper" as CFString, authRef, &error) == false {
    ...
    private func helperConnection() -> NSXPCConnection? {
        ...
        let connection = NSXPCConnection(machServiceName: "nu.soep.deadair.SMC.Helper", options: .privileged)
...
/// The menu bar item shown while the app is paused, so there is still something to click
/// when every module's widget is hidden. It draws the identity mark rather than the plate
/// with two coloured traces it inherited, which was the previous project's app icon.
public class AppIcon: NSView {
    ...
    public override func draw(_ dirtyRect: NSRect) {
        super.draw(dirtyRect)
        // Muted, because a paused app is not passing anything through.
        Brand.drawMark("flatline", in: self.bounds, color: Brand.legendPrimary)
    }
}
...
public func restartApp(_ sender: Any, afterDelay seconds: TimeInterval = 0.5) -> Never {
    // The delay matters: this instance has to be gone before the next one starts. The
    // shell stays, but the two values arrive as positional arguments instead of being
    // interpolated into the script, so a $(...) in a parent directory name is data.
    let task = Process()
    task.executableURL = URL(fileURLWithPath: "/bin/sh")
    task.arguments = [
        "-c", "sleep \"$1\"; open -n \"$2\"",
        "sh", String(seconds), Bundle.main.bundlePath
    ]
    try? task.run()
...
public class StepperInput: NSStackView, NSTextFieldDelegate, PreferencesSwitchWith_p {
    ...
    public init(
        ...
        self.valueView.font = Brand.Font.utility(size: 12)
...
public class SeparatorView: NSStackView {
    public init(label: String, button: NSView? = nil) {
        ...
        labelView.isEditable = false
        labelView.drawsBackground = false
        labelView.attributedStringValue = Brand.cardTitle(label)
